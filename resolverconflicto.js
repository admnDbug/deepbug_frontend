document.addEventListener('DOMContentLoaded', () => {
    ...
    async function cargarConflictos() {
        try {
            const respuesta = await fetch(`http://localhost:3000/api/protocolos/${proyectoId}`, {
    ...
    window.resolver = async function(idProtocoloEnConflicto, accion) {
        ...
        try {
            const respuesta = await fetch(`http://localhost:3000/api/protocolos/resolver/${idProtocoloEnConflicto}`, {
                method: 'PUT',
                headers: {
