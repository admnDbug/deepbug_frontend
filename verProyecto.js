document.addEventListener('DOMContentLoaded', () => {
    ...
    async function cargarVistaProyecto() {
        try {
            // Petición 1: Proyecto
            const resProyecto = await fetch(`http://localhost:3000/api/biomonitoreos/${proyectoId}`, {
            ...
            let protocolos = [];
            try {
                const resProtocolos = await fetch(`http://localhost:3000/api/protocolos/${proyectoId}`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
    ...
    async function removerColaborador(colaborador_id) {
        try {
            const res = await fetch(`http://localhost:3000/api/biomonitoreos/${proyectoId}/remover-colaborador`, {
                method: 'PUT',
                headers: {
    ...
    if (btnEliminarProyecto) {
        btnEliminarProyecto.addEventListener('click', async () => {
            ...
            try {
                ...
                const res = await fetch(`http://localhost:3000/api/biomonitoreos/${proyectoId}`, {
                    method: 'DELETE',
                    headers: { 'Authorization': `Bearer ${token}` }
                });
    ...
    if (btnSalirProyecto) {
        btnSalirProyecto.addEventListener('click', async () => {
            ...
            try {
                ...
                const res = await fetch(`http://localhost:3000/api/biomonitoreos/${proyectoId}/salir`, {
                    method: 'PUT',
                    headers: { 'Authorization': `Bearer ${token}` }
                });
